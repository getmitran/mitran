...
export default function InitWizard({ open, onClose }: Props) {
  const [form, setForm] = useState({ team_name: '', description: '', languages: '', team_size: 5 })
      ...
      <div className="bg-surface border border-gray-800 rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-semibold text-gray-100 flex items-center gap-2">
            <Rocket size={18} className="text-accent" /> Initialize Team
          </h2>
          ...
          <>
            <div className="flex flex-col gap-3 mb-4">
              <input value={form.team_name} onChange={e => setForm(f => ({ ...f, team_name: e.target.value }))}
                placeholder="Team name" className="bg-base border border-gray-800 rounded-md px-3 py-2 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-accent/50" />
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder="What does this team build?" className="bg-base border border-gray-800 rounded-md px-3 py-2 text-sm text-gray-200 placeholder-gray-600 resize-none h-20 focus:outline-none focus:border-accent/50" />
            ...
            <button onClick={submit} disabled={status === 'loading' || !form.team_name}
