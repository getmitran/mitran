...
type Employee struct {
	ID, Name, Email, Department, Title, ManagerID, Phone, Location, JoinDate string
	Active                                                                    bool
}
...
func (d *Directory) Get(id string) *Employee {
	...
	if e, ok := d.emps[id]; ok { return &e }; return nil
}

func (d *Directory) Search(query string) []Employee {
	d.mu.RLock(); defer d.mu.RUnlock()
	q := strings.ToLower(query); var res []Employee
...
func (d *Directory) ListByDepartment(dept string) []Employee {
	d.mu.RLock(); defer d.mu.RUnlock()
	var res []Employee
	for _, e := range d.emps { if strings.EqualFold(e.Department, dept) { res = append(res, e) } }
	return res
}
...
func (d *Directory) ListByManager(managerID string) []Employee {
	d.mu.RLock(); defer d.mu.RUnlock()
	var res []Employee
	for _, e := range d.emps { if e.ManagerID == managerID { res = append(res, e) } }
	return res
}
...
func (d *Directory) Handler(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == "POST":
		var e Employee
		json.NewDecoder(r.Body).Decode(&e); d.Add(e)
		w.WriteHeader(201); json.NewEncoder(w).Encode(e)
	case r.Method == "GET" && path != "" && path != "/":
		if e := d.Get(strings.TrimPrefix(path, "/")); e != nil { json.NewEncoder(w).Encode(e) } else { http.NotFound(w, r) }
	case r.Method == "GET":
		if q := r.URL.Query().Get("q"); q != "" { json.NewEncoder(w).Encode(d.Search(q)) } else {
			d.mu.RLock(); all := make([]Employee, 0, len(d.emps))
			for _, e := range d.emps { all = append(all, e) }; d.mu.RUnlock()
